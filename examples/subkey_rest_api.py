#!/usr/bin/env python3
"""
Example script demonstrating L2-only REST API usage with linoraSubkey.
...
from linora_py import linora, linoraSubkey
...
public_linora = linora(env=TESTNET, logger=logger)
system_state = public_linora.api_client.fetch_system_state()
logger.info(f"{system_state=}")
system_time = public_linora.api_client.fetch_system_time()
logger.info(f"{system_time=}")
insurance_fund = public_linora.api_client.fetch_insurance_fund()
logger.info(f"{insurance_fund=}")
markets = public_linora.api_client.fetch_markets()
...
for market in markets["results"][:5]:  # Limit to 5 markets for testing
    ...
    mkt_summary = public_linora.api_client.fetch_markets_summary({"market": symbol})
    logger.info(f"{mkt_summary=}")
    ob = public_linora.api_client.fetch_orderbook(market=symbol, params={"depth": 5})
    logger.info(f"{ob}=")
    bbo = public_linora.api_client.fetch_bbo(market=symbol)
    logger.info(f"{bbo=}")

    try:
        trades = public_linora.api_client.fetch_trades({"market": symbol, "page_size": 20})
        logger.info(f"{trades=}")
    except Exception as e:
        logger.warning(f"Could not fetch trades for {symbol}: {e}")

    try:
        funding_data = public_linora.api_client.fetch_funding_data(params={"market": symbol})
...
system_config = public_linora.api_client.fetch_system_config()
logger.info(f"{system_config=}")

# Test Private API calls with L2-only authentication using linoraSubkey
linora = linoraSubkey(
    env=TESTNET,
...
account_summary = linora.api_client.fetch_account_summary()
logger.info(f"{account_summary=}")
account_profile = linora.api_client.fetch_account_profile()
logger.info(f"{account_profile=}")

try:
    balances = linora.api_client.fetch_balances()
    logger.info(f"{balances=}")
except Exception as e:
    logger.warning(f"Could not fetch balances: {e}")

try:
    positions = linora.api_client.fetch_positions()
    logger.info(f"{positions=}")
except Exception as e:
    logger.warning(f"Could not fetch positions: {e}")

try:
    transactions = linora.api_client.fetch_transactions(params={"page_size": 5})
    logger.info(f"{transactions=}")
except Exception as e:
    logger.warning(f"Could not fetch transactions: {e}")

try:
    fills = linora.api_client.fetch_fills(params={"page_size": 5})
    logger.info(f"{fills=}")
except Exception as e:
    logger.warning(f"Could not fetch fills: {e}")

try:
    tradebusts = linora.api_client.fetch_tradebusts()
    logger.info(f"{tradebusts=}")
except Exception as e:
    logger.warning(f"Could not fetch tradebusts: {e}")

try:
    hist_orders = linora.api_client.fetch_orders_history(params={"page_size": 5})
    logger.info(f"{hist_orders=}")
except Exception as e:
    logger.warning(f"Could not fetch orders history: {e}")

try:
    subaccounts = linora.api_client.fetch_subaccounts()
    logger.info(f"{subaccounts=}")
except Exception as e:
    logger.warning(f"Could not fetch subaccounts: {e}")

try:
    account_info = linora.api_client.fetch_account_info()
    logger.info(f"{account_info=}")
except Exception as e:
    logger.warning(f"Could not fetch account info: {e}")

points_program = linora.api_client.fetch_points_data(
    market="ETH-USD-PERP",
    program="Maker",
)
logger.info(f"Maker {points_program=}")

transfers = linora.api_client.fetch_transfers(params={"page_size": 5})
...
for market in markets["results"][:5]:  # Limit to 5 markets for testing
    ...
    symbol = market["symbol"]
    try:
        orders = linora.api_client.fetch_orders(params={"market": symbol})
        logger.info(f"{symbol=} {orders=}")
    except Exception as e:
        logger.warning(f"Could not fetch orders for {symbol}: {e}")

    try:
        fills = linora.api_client.fetch_fills(params={"market": symbol, "page_size": 5})
        logger.info(f"{symbol=} {fills=}")
    except Exception as e:
        logger.warning(f"Could not fetch fills for {symbol}: {e}")

    try:
        funding_payments = linora.api_client.fetch_funding_payments(params={"market": symbol})
...
response = linora.api_client.submit_order(order=buy_order)
buy_id = response.get("id")
logger.info(f"Buy Order {response=}")
if buy_id is None:
    logger.error("Failed to get buy order ID")
    exit(1)
buy_order_status = linora.api_client.fetch_order_by_client_id(client_id=buy_client_id)
...
response = linora.api_client.submit_order(order=sell_order)
logger.info(f"Sell Order {response=}")
sell_id = response.get("id")
if sell_id is None:
    logger.error("Failed to get sell order ID")
    exit(1)
sell_order_status = linora.api_client.fetch_order(order_id=sell_id)
logger.info(f"{sell_order_status=}")

# Check all open orders
orders = linora.api_client.fetch_orders()
...
response = linora.api_client.modify_order(buy_id, modify_order)
logger.info(f"Modify order response {response}")

# Cancel ETH open order
linora.api_client.cancel_all_orders({"market": "ETH-USD-PERP"})
orders = linora.api_client.fetch_orders()
logger.info(f"After ETH-USD-PERP Cancel {orders=}")
linora.api_client.cancel_order(order_id=buy_id)
orders = linora.api_client.fetch_orders()
...
response = linora.api_client.submit_orders_batch(orders=orders)
