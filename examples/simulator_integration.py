#!/usr/bin/env python3
"""
linora Simulator Integration Example
...
try:
    from linora_py.api.ws_models import (
        JSONRPCRequest,
        create_data_message,
...
async def demo_full_integration():
    ...
    # Full linora instance with all injections
    linora = linora(
        env=Environment.TESTNET,
        # REST injection
    ...
    await linora.ws_client.subscribe_by_name("bbo.BTC-USD-PERP", update_handler)

    # Process all messages
    while len(updates) < 5:
        result = await linora.ws_client.pump_once()
        if not result:
            break

    print(f"✅ WebSocket: {len(updates)} BBO updates received")
    await linora.ws_client._close_connection()


async def main():
    """Run all simulator integration examples."""
    print("🚀 linora Simulator Integration")
    print("=" * 50)
