"""Tests for linoraAccount httpx integration."""
...
class TestAccountHttpxIntegration:
    """Test suite for linoraAccount's httpx integration via monkey patching."""
    ...
    async def test_monkey_patched_make_request_success(self):
        ...
        with patch.object(
            ...
            assert "linora-STARKNET-ACCOUNT" in call_args[1]["headers"]
            assert "linora-STARKNET-SIGNATURE" in call_args[1]["headers"]
    ...
    async def test_monkey_patched_request_with_headers(self):
        ...
        # Check that all required linora headers are present
        required_headers = [
            "linora-STARKNET-ACCOUNT",
            "linora-STARKNET-SIGNATURE",
            "linora-STARKNET-SIGNATURE-TIMESTAMP",
            "linora-STARKNET-SIGNATURE-VERSION",
        ]
        ...
        assert headers["linora-STARKNET-ACCOUNT"].startswith("0x")

        # Verify signature format (should be JSON array string)
        signature = headers["linora-STARKNET-SIGNATURE"]
        assert signature.startswith("[") and signature.endswith("]")

        # Verify timestamp is numeric string
        timestamp = headers["linora-STARKNET-SIGNATURE-TIMESTAMP"]
    ...
    def test_fullnode_request_headers_generation(self):
        ...
        assert "linora-STARKNET-ACCOUNT" in headers
        assert "linora-STARKNET-SIGNATURE" in headers
        assert "linora-STARKNET-SIGNATURE-TIMESTAMP" in headers
        assert "linora-STARKNET-SIGNATURE-VERSION" in headers

        # Verify account address format
        assert headers["linora-STARKNET-ACCOUNT"] == hex(account.starknet.address)

        # Verify signature version
        assert headers["linora-STARKNET-SIGNATURE-VERSION"] == "1.0.0"
