...
class TestWebSocketReconnect:
    ...
    def setup_method(self):
        ...
        self.ws_client = linoraWebsocketClient(env=TESTNET, auto_start_reader=False)
    ...
    def test_configurable_sleep_durations(self):
        ...
        client_custom = linoraWebsocketClient(
            env=TESTNET,
            auto_start_reader=False,
    ...
    def test_ping_interval_configuration(self):
        ...
        client = linoraWebsocketClient(env=TESTNET, auto_start_reader=False, ping_interval=30.0)
        assert client.ping_interval == 30.0

        # Test default ping interval (None)
        client_default = linoraWebsocketClient(env=TESTNET, auto_start_reader=False)
    ...
    def test_disable_reconnect_configuration(self):
        ...
        client = linoraWebsocketClient(env=TESTNET, auto_start_reader=False, disable_reconnect=True)
    ...
    def test_ws_url_override(self):
        ...
        client = linoraWebsocketClient(env=TESTNET, auto_start_reader=False, ws_url_override=custom_url)
    ...
    def test_validate_messages_configuration(self):
        ...
        linoraWebsocketClient(env=TESTNET, auto_start_reader=False, validate_messages=True)
    ...
    async def test_concurrent_recv_race_condition_fixed(self):
        ...
        client = linoraWebsocketClient(env=TESTNET, auto_start_reader=True, connector=mock_connector)
        await client.connect()
...
class TestnetLikeWebSocket:
    ...
    async def test_comprehensive_reconnect_with_mocked_failure(self):
        ...
        try:
            ...
            await client.subscribe(
                linoraWebsocketChannel.TRADES, callback=trades_handler, params={"market": "BTC-USD-PERP"}
            )

            # Subscribe to markets summary
            await client.subscribe(
                linoraWebsocketChannel.MARKETS_SUMMARY, callback=markets_summary_handler, params={"market": "ALL"}
            )
    ...
    async def test_real_testnet_integration(self):
        ...
        try:
            ...
            await client.subscribe(
                linoraWebsocketChannel.MARKETS_SUMMARY, callback=message_handler, params={"market": "ALL"}
            )
...
class TestWebSocketMessageValidation:
    ...
    async def test_message_validation_enabled(self):
        ...
        client = linoraWebsocketClient(env=TESTNET, auto_start_reader=False, validate_messages=True)
        ...
        with patch("linora_py.api.ws_client.validate_ws_message") as mock_validate:
    ...
    async def test_message_validation_disabled(self):
        ...
        client = linoraWebsocketClient(env=TESTNET, auto_start_reader=False, validate_messages=False)
