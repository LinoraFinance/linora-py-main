"""Tests for WebSocket compression support in linoraWebsocketClient."""
...
class TestWebSocketCompression:
    ...
    def test_compression_explicitly_enabled(self):
        ...
        ws_client = linoraWebsocketClient(env=TESTNET, enable_compression=True)
    ...
    def test_compression_disabled(self):
        ...
        ws_client = linoraWebsocketClient(env=TESTNET, enable_compression=False)
    ...
    async def test_compression_with_custom_connector(self, mock_connect: AsyncMock):
        ...
        ws_client = linoraWebsocketClient(env=TESTNET, connector=custom_connector, enable_compression=False)
        await ws_client.connect()
    ...
    def test_compression_disabled_via_linora_constructor(self):
        """Test that compression can be disabled via linora constructor."""
        linora = linora(env=TESTNET, enable_ws_compression=False)

        # Should have created a WebSocket client with compression disabled
        assert linora.ws_client.enable_compression is False

    def test_compression_enabled_via_linora_constructor(self):
        """Test that compression is enabled via linora constructor."""
        linora = linora(env=TESTNET, enable_ws_compression=True)

        # Should have created a WebSocket client with compression enabled
        assert linora.ws_client.enable_compression is True

    def test_compression_default_via_linora_constructor(self):
        """Test that compression is enabled by default via linora constructor."""
    ...
    async def test_compression_with_ping_interval(self, mock_connect: AsyncMock):
        ...
        ws_client = linoraWebsocketClient(env=TESTNET, enable_compression=False, ping_interval=15.0)
        await ws_client.connect()
