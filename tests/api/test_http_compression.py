...
class TestHttpCompression:
    ...
    def test_compression_disabled_via_linora_constructor(self):
        ...
        linora = linora(env=TESTNET, enable_http_compression=False)

        # Should have created an HttpClient with compression disabled
        assert linora.api_client.client.headers["Accept-Encoding"] == "identity"

    def test_compression_enabled_via_linora_constructor(self):
        """Test that compression is enabled via linora constructor."""
        linora = linora(env=TESTNET, enable_http_compression=True)

        # Should not have Accept-Encoding set to identity
        assert (
            "Accept-Encoding" not in linora.api_client.client.headers
            or linora.api_client.client.headers["Accept-Encoding"] != "identity"
        )
    ...
    def test_compression_default_via_linora_constructor(self):
        ...
        # Note: linora doesn't create HttpClient when all parameters are default
