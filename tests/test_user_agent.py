...
class TestUserAgent:
    ...
    def test_user_agent_format(self):
        ...
        assert "linora-py/" in user_agent
    ...
    def test_user_agent_includes_sdk_version(self):
        ...
        # Should have version after linora-py/
        assert user_agent.startswith("linora-py/")
        version_part = user_agent.split("linora-py/")[1].split(" ")[0]
    ...
    def test_user_agent_fallback_dev_version(self):
        ...
        with patch("linora_py.user_agent.version", side_effect=PackageNotFoundError("Package not found")):
            user_agent = get_user_agent()

            # Should fallback to "dev"
            assert "linora-py/dev" in user_agent

    def test_user_agent_structure(self):
        """Test that user agent has correct structure."""
        user_agent = get_user_agent()

        # Expected format: "linora-py/{VERSION} (Python {PYTHON_VERSION}; {PLATFORM})"
        parts = user_agent.split(" ", 1)
        assert len(parts) == 2
        assert parts[0].startswith("linora-py/")
