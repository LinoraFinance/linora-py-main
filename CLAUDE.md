...
# Type checking
uv run mypy --check-untyped-defs linora_py
```
...
1. **linora Client** (`linora_py/linora.py`): Main entry point
...
2. **Account Management** (`linora_py/account/`):
...
4. **Message Signing** (`linora_py/message/`):
...
- Uses `linora_py.environment.Environment` enum: `PROD` or `TESTNET`
