...
from linora_py.api.block_trades_api import BlockTradesMixin
from linora_py.api.http_client import HttpClient, HttpMethod
from linora_py.api.models import AccountSummary, AccountSummarySchema, AuthSchema, SystemConfig, SystemConfigSchema
from linora_py.api.protocols import AuthProvider, RetryStrategy, Signer
...
class linoraApiClient(BlockTradesMixin, HttpClient):
    ...
    def __init__(
        ...
        if api_base_url is not None:
            ...
            self.api_url = f"https://api.{self.env}.linora.trade/v1"
    ...
    def init_account(self, account: linoraAccount):
    ...
    def set_token(self, jwt: str) -> None:
        ...
        Used by ``linoraApiKey`` to supply a pre-generated token, and also useful
    ...
    def fetch_order(self, order_id: str) -> dict:
        ...
        Args:
            order_id: order's id as assigned by linora.
        """
    ...
    def fetch_trades(self, params: dict) -> dict:
        """Fetch linora exchange trades for specific market.
    ...
    def submit_orders_batch(self, orders: list[Order], signer: Signer | None = None) -> dict:
        """Send batch of orders to linora.
            Private endpoint requires authorization.
    ...
    def modify_order(self, order_id: str, order: Order, signer: Signer | None = None) -> dict:
        """Modify an open order previously sent to linora from this account.
            Private endpoint requires authorization.
    ...
    def cancel_order(self, order_id: str) -> None:
        """Cancel open order previously sent to linora from this account.
            Private endpoint requires authorization.
    ...
    def cancel_orders_batch(
        ...
        Args:
            order_ids: List of order IDs assigned by linora
    ...
    def fetch_system_config(self) -> SystemConfig:
        """Fetch linora system config.

        Examples:
            >>> linora.api_client.fetch_system_config()
    ...
    def fetch_system_state(self) -> dict:
        """Fetch linora system status.

        Examples:
            >>> linora.api_client.fetch_system_state()
            >>> { "status": "ok" }
        """
        return self._get(path="system/state")

    def fetch_system_time(self) -> dict:
        """Fetch linora system time.

        Examples:
            >>> linora.api_client.fetch_system_time()
            >>> { "server_time": "1710956478221" }

        Returns:
            server_time: linora Server time
        """
