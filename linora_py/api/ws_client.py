...
from linora_py.user_agent import get_user_agent

# Optional typed message models
try:
    from linora_py.api.ws_message_models import validate_ws_payload
    from linora_py.api.ws_models import validate_ws_message
...
class linoraWebsocketChannel(Enum):
    """Enum class to define the channels for linora Websocket API.
...
def _linora_channel_prefix(value: str) -> str:
    return value.split(".")[0]


def _get_ws_channel_from_name(message_channel: str) -> linoraWebsocketChannel | None:
    for channel in linoraWebsocketChannel:
        if message_channel.startswith(_linora_channel_prefix(channel.value)):
            return channel
    return None


class linoraWebsocketClient:
    """Class to interact with linora WebSocket JSON-RPC API.
    ...
    Args:
        ...
        api_client (Optional[Any], optional): Reference to linoraApiClient for token refresh. Defaults to None.

    Examples:
        >>> from linora_py import linora
        >>> from linora_py.environment import Environment
        >>> linora = linora(env=Environment.TESTNET)
        >>> linora.ws_client.connect()
        >>> # With custom timeout
        >>> from linora_py.api.ws_client import linoraWebsocketClient
        >>> ws_client = linoraWebsocketClient(env=Environment.TESTNET, ws_timeout=30)
        >>> # With manual pumping disabled
        >>> ws_client = linoraWebsocketClient(env=Environment.TESTNET, auto_start_reader=False)
        >>> # High-frequency simulator mode (no sleeps)
        >>> ws_client = linoraWebsocketClient(env=Environment.TESTNET,
        ...                                   reader_sleep_on_error=0, reader_sleep_on_no_connection=0)
        >>> # With typed message validation
        >>> ws_client = linoraWebsocketClient(env=Environment.TESTNET, validate_messages=True)
    ...
    classname: str = "linoraWebsocketClient"
    ...
    def __init__(
        ...
        self.env = env
        self.api_url = ws_url_override or f"wss://ws.api.{self.env}.linora.trade/v1"
    ...
    def init_account(self, account: linoraAccount) -> None:
    ...
    async def connect(self) -> bool:
        """Connect to linora WebSocket API.
    ...
    async def _process_message(self, response: str) -> None:
        ...
        if "params" not in message:
            ...
            ws_channel: linoraWebsocketChannel | None = _get_ws_channel_from_name(message_channel)
    ...
    async def subscribe(
        self,
        channel: linoraWebsocketChannel,
        callback: Callable,
        ...
        Args:
            channel (linoraWebsocketChannel): Channel to subscribe
        ...
        if channel == linoraWebsocketChannel.MARKETS_SUMMARY and not params:
            params = {"market": "ALL"}

        # Handle ORDER_BOOK channel with optional parameters
        if channel == linoraWebsocketChannel.ORDER_BOOK:
