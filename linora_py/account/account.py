...
from linora_py.account.starknet import Account as StarknetAccount
from linora_py.account.utils import derive_stark_key, derive_stark_key_from_ledger, flatten_signature
from linora_py.api.models import SystemConfig
from linora_py.common.order import Order
from linora_py.message.auth import build_auth_message, build_fullnode_message
from linora_py.message.block_trades import BlockTrade, build_block_trade_message
from linora_py.message.onboarding import build_onboarding_message
from linora_py.message.order import build_modify_order_message, build_order_message
from linora_py.message.stark_key import build_stark_key_message
...
class linoraAccount:
    """Class to generate and manage linora account.
    ...
    Args:
        ...
        l2_private_key (Optional[str], optional): linora private key. Defaults to None.
    ...
    Examples:
        ...
        >>> linora.account.l2_address
        >>> linora.account.l2_public_key
        >>> linora.account.l2_private_key
    """
    ...
    def __init__(
        ...
        if l1_address is None:
            raise_value_error("linora: Provide Ethereum address")
        ...
        if l1_private_key is not None:
            ...
            raise_value_error("linora: Provide Ethereum or linora private key")
    ...
    # to inject http headers requested by linora full node:
    # - linora-STARKNET-ACCOUNT: account address signing the request
    # - linora-STARKNET-SIGNATURE: signature of the request
    # - linora-STARKNET-SIGNATURE-TIMESTAMP: timestamp of the signature
    # - linora-STARKNET-SIGNATURE-VERSION: version of the signature
    def _apply_fullnode_headers_patch(self, client):
        """Apply the fullnode headers patch for linora-specific headers."""
        current_self = self
    ...
    def onboarding_signature(self) -> str:
        if self.config is None:
            raise_value_error("linora: System config not loaded")
    ...
    def onboarding_headers(self) -> dict:
        return {
            "linora-ETHEREUM-ACCOUNT": self.l1_address,
            "linora-STARKNET-ACCOUNT": hex(self.l2_address),
            "linora-STARKNET-SIGNATURE": self.onboarding_signature(),
        }
    ...
    def auth_headers(self) -> dict:
        ...
        return {
            "linora-STARKNET-ACCOUNT": hex(self.l2_address),
            "linora-STARKNET-SIGNATURE": self.auth_signature(timestamp, expiry),
            "linora-TIMESTAMP": str(timestamp),
    ...
    def fullnode_request_headers(self, account: StarknetAccount, chain_id: int, json_payload: str):
        ...
        return {
            ...
            "linora-STARKNET-SIGNATURE-TIMESTAMP": str(signature_timestamp),
            "linora-STARKNET-SIGNATURE-VERSION": FULLNODE_SIGNATURE_VERSION,
        }
