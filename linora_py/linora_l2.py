...
from linora_py.account.subkey_account import SubkeyAccount
...
class linoraL2(_ClientBase):
    ...
    Args:
        ...
        l2_address (str): L2 address of the account that owns this key.
            ...
            For a registered subkey (``linoraSubkey``), this must be the
    ...
    Examples:
        >>> from linora_py import linoraL2
    ...
    def __init__(
        ...
        _validate_env(env, "linoraL2")

        if not l2_private_key:
            raise_value_error(f"linoraL2: L2 private key is required, got {l2_private_key!r}")
        if not l2_address:
            raise_value_error(f"linoraL2: L2 address is required, got {l2_address!r}")

        self.env = env
        self.logger: logging.Logger = logger or logging.getLogger(__name__)

        self.api_client = linoraApiClient(env=env, logger=logger)
